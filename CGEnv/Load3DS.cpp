...
CLoad3DS::CLoad3DS()
{
	...
	if (m_CurrentChunk==0){
        msgBox.setText("ERROR: No hi ha prouta memoria");
		msgBox.exec();
		exit(-1);
	}
...
//		msgBox.setText("No hi ha prouta memoria. Error!");
        sprintf(strMessage, "No hi ha prouta memoria. Error!");
		msgBox.setText(strMessage);
		msgBox.exec();
		exit(-1);
	}
...
//				MessageBox(NULL, "This 3DS file is over version 3 so it may load incorrectly", "Warning", MB_OK);
                sprintf(strMessage, "This 3DS file is over version 3 so it may load incorrectly");
				msgBox.setText(strMessage);
                msgBox.exec(); }
			break;
...
void CLoad3DS::ReadVertexIndices(t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	if (pObject->pFaces==0){
//		MessageBox(NULL, "No hi ha prouta memoria", "Error!", MB_OK);
		msgBox.setText("No hi ha prouta memoria");
		msgBox.exec();
		exit(-1);
	}
