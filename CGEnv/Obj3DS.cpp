...
//
bool Obj_3DS::Carregar3DS(const char *nom)
{
...
// Afegir base path al fitxer de textura
			...
			strcat(szLibraryFile, g_3DModel.pMaterials[i].strFile);
			g_Texture[i] = loadIMA2(szLibraryFile);	// Load The Images with base path
			struct nomfitxer a;
			a.num=i;
            strcpy(a.nom,g_3DModel.pMaterials[i].strFile);
					...
					for(int whichVertex = 0; whichVertex < 3; whichVertex++)
					{
						...
						if(pObject->bHasTexture) {
							...
							if(g_3DModel.pMaterials.size() && pObject->materialID >= 0) 
							{
                                ...
                                unsigned char *pColor = g_3DModel.pMaterials[pObject->materialID].color;
...
GLuint Obj_3DS::loadIMA2(char* filename)
{
	GLuint texID;
	QImage t;
	QImage b;
