...
bool COBJModel::LoadModel(const char *szFileName,unsigned int iDisplayList)
{
	////////////////////////////////////////////////////////////////////////
	...
	while (!feof(hFile))
	{
		...
		if (pMaterials)
		{
			...
			if (!strncmp(szString, MTL_LIB_ID, sizeof(MTL_LIB_ID)))
			{
                ...
                // Eliminar el segon caracter de fi de linia (\r\n) quan es llegeix fitxers texte en Mac OS
                if (szString[strlen(szString) - 1] == char('\r')) szString[strlen(szString) - 1] = char('\0');
...
void COBJModel::ParseFaceString(char szFaceString[], Face *FaceOut,
...
bool COBJModel::LoadMaterialLib(const char szFileName[],
								Material *pMaterials, 
								...
								char szBasePath[])
{
	...
	while (!feof(hFile))
	{
		...
		if (!strncmp(szString, MTL_TEXTURE_ID, sizeof(MTL_TEXTURE_ID)))
		{
            ...
            pMaterials[*iCurMaterialIndex].iTextureID = LoadTextureQt(szTextureFile);
		}
...
void COBJModel::RenderToDisplayList(const Face *pFaces,
...
void COBJModel::UseMaterial(const Material *pMaterial)
{
	////////////////////////////////////////////////////////////////////////
...
void  COBJModel::GetFaceNormal(float fNormalOut[], const Face *pFace)
{
	////////////////////////////////////////////////////////////////////////
...
void COBJModel::GetFileInfo(FILE *hStream, OBJFileInfo *Info,
...
//    FILE *hMaterialLib=NULL;
	...
	while (!feof(hStream))
	{
		...
		if (!strncmp(szString, MTL_LIB_ID, sizeof(MTL_LIB_ID)))
		{
            ...
            QFileInfo cami(szBasePath);

            QByteArray a=cami.absoluteFilePath().toLatin1();
            char *szFilePath=a.data();

			// Open the library file
            FILE *hMaterialLib = fopen(szFilePath, "r");
//            FILE *hMaterialLib = fopen(szBasePath, "r");
//          hMaterialLib = fopen(szBasePath,"r");
			// Success ?
			if (hMaterialLib)
			{
				// Quit reading when end of file has been reached
...
void COBJModel::ReadNextString(char szString[], FILE *hStream)
{
	////////////////////////////////////////////////////////////////////////
...
void COBJModel::MakePath(char szFileAndPath[])
{
	////////////////////////////////////////////////////////////////////////
...
void COBJModel::GetTokenParameter(char szString[],
...
// Carrega textures fitxers OBJ en Qt
int COBJModel::LoadTextureQt(const char szFileName[_MAX_PATH])
{
	////////////////////////////////////////////////////////////////////////
...
/// Open the image file for reading
    file=fopen(szFileName,"r");					// Funció Visual Studio 6.0
...
void COBJModel::EliminaLlista(unsigned int iDisplayList)
{
